...
For calculating the trimmed MSE we use the following function
```{r, echo=TRUE}
...
Since the trimmed mean leaves out the worst errors the trimmed MSE is much better then the untrimmed. MSE values are also always worse for the test-set.

## b)

We plot the calculated MSEs in one parallel boxplot
...
boxplot(bs$mse_train, bs$mse_test, names=c("bootstraped", "left-out"), xlab="Data-sets", ylab="MSE", main="MSEs with Bootstrap")

```
The MSE values have a big variance with maximum values reaching to 20. In order to compare MSE values, here is the plot without outliers.

```{r, echo=TRUE}
boxplot(bs$mse_train, bs$mse_test, names=c("bootstraped", "left-out"), outline=FALSE, xlab="Data-sets", ylab="MSE", main="MSEs with Bootstrap (without outliers)")
